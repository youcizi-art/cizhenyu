...
import { PluginService } from "./PluginService_DZ2-uZAU.mjs";
...
import { A as AgentCapabilityService, a as aiSkills } from "./AgentCapabilityService_4lssCQgg.mjs";
...
auth.post("/admin/login", async (c) => {
  ...
  const { TurnstileService } = await import("./TurnstileService_DoDAnzuk.mjs");
...
auth.post("/member/send-code", async (c) => {
  ...
  const { MailService } = await import("./MailService_CtsXg8Ec.mjs");
...
rbac.get("/permissions", async (c) => {
  const db = await createDbClient(c.env.DB);
  const { plugins: pluginsTable } = await import("./index_Bs1dLHcd.mjs").then((n) => n.bR);
...
class CloudflareService {
  ...
  static async syncSecurityDomains(env, config) {
    ...
    if (config.admin_domain) collected.add(config.admin_domain);
    if (config.api_domain) collected.add(config.api_domain);
    if (config.member_domain) collected.add(config.member_domain);
    if (config.public_domains) {
      config.public_domains.forEach((d) => collected.add(d));
    }
...
users.get("/balance/logs", requirePermission("user.balance_manage"), async (c) => {
  ...
  if (search) {
    const { or, like } = await import("./domains_JCVeAtQW.mjs").then((n) => n.a);
    whereClause = or(
  ...
  const { count } = await import("./domains_JCVeAtQW.mjs").then((n) => n.a);
...
function resolveTenantId(c) {
  ...
  return Number(domains?.tenant_id || domains?.id || (domains?.site_domains ? domains.site_domains.id : 0) || 0);
}
...
dbAdmin.use("*", async (c, next) => {
  ...
  if (!userPermissions || !userRoles) {
    try {
      const { createDbClient: createDbClient2 } = await import("./index_Bs1dLHcd.mjs").then((n) => n.bS);
      const { getAdminAuthInfo: getAdminAuthInfo2 } = await import("./rbac_ubixCz_c.mjs");
...
async function primeRuntimePermissions(db, registry2) {
  ...
  const { PluginService: PluginService2 } = await import("./PluginService_DZ2-uZAU.mjs");
  const { PLUGIN_CODE_REGISTRY: PLUGIN_CODE_REGISTRY2 } = await import("./plugin-registry_DWil8Y54.mjs");
...
async function performSystemSync(c, registry2) {
  const db = await createDbClient(c.env.DB);
  ...
  try {
    ...
    try {
      ...
      for (const p of enabledPlugins) {
        const bundle = PLUGIN_CODE_REGISTRY2[p.slug];
        if (bundle) {
          const manifest = await bundle.getManifest();
          if (manifest && manifest.permissions) {
            registry2.registerPluginPermissions(p, manifest.permissions);
          }
          const initFn = await bundle.getInit();
          if (typeof initFn === "function") {
            try {
              await initFn(db);
              console.log(`🔌 [System] 插件 ${p.slug} 数据库自愈同步成功.`);
            } catch (initErr) {
              console.warn(`⚠️ [System] 插件 ${p.slug} 数据库自愈同步失败:`, initErr);
            }
          }
        }
      }
...
function createAdminApp() {
  ...
  auth.get("/seed-users", async (c) => {
    ...
    try {
      const dbModule = await import("./index_Bs1dLHcd.mjs").then((n) => n.bS);
      const db = await dbModule.createDbClient(c.env.DB);
      const schema = dbModule.schema;
      const { generateId: generateId2 } = await import("./index_EBjtrVuP.mjs");
      const { passwordHasher: passwordHasher2 } = await import("./auth_C9MZO_Gl.mjs").then((n) => n.d);
  ...
  v1.all("/s/*", async (c) => {
    const sf = (await import("./front-api_CUoWtDtR.mjs")).default;
    const url = new URL(c.req.url);
  ...
  master.use("*", async (c, next) => {
    try {
      ...
      if (c.env && !c.env.__executionCtx) {
        c.env.__executionCtx = getSafeExecutionCtx(c);
      }
      if (!isSynced && !syncPromise) {
        const syncTask = shouldRunFullRuntimeSync(c.env) ? performSystemSync(c, registry$1) : primeRuntimePermissions(await createDbClient(c.env.DB), registry$1);
        syncPromise = syncTask.finally(() => {
          syncPromise = null;
        });
      }
      if (syncPromise) {
