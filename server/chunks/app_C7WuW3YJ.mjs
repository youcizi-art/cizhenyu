...
import { P as PLUGIN_CODE_REGISTRY } from "./auto-registry.gen_BIe-yiWO.mjs";
...
async function primeRuntimePermissions(db, registry2) {
  ...
  const { PluginService: PluginService2 } = await import("./PluginService_D9tcZttx.mjs");
  const { PLUGIN_CODE_REGISTRY: PLUGIN_CODE_REGISTRY2 } = await import("./plugin-registry_DGoeEBJM.mjs");
...
function createAdminApp() {
  ...
  v1.all("/s/*", async (c) => {
    const sf = (await import("./front-api_BvixIQUc.mjs")).default;
    const url = new URL(c.req.url);
