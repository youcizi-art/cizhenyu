...
const onRequest$1 = defineMiddleware(async (context, next) => {
  ...
  try {
    ...
    if (cloudflareWorkers && cloudflareWorkers.env) {
      DB = cloudflareWorkers.env.DB;
      NS_CONFIG = cloudflareWorkers.env.NS_CONFIG;
      workersEnv = cloudflareWorkers.env;
    } else {
      throw new Error("env not found in cloudflare:workers");
    }
  } catch (e) {
    const runtime = context.locals.runtime;
    DB = runtime?.DB || context.locals.DB;
    NS_CONFIG = runtime?.NS_CONFIG || context.locals.NS_CONFIG;
    workersEnv = runtime?.env || context.locals.env || {};
  }
  let config = null;
  let target = "public";
  if (DB) {
    const envObj = { ...workersEnv, DB, NS_CONFIG };
