...
async function executeGetInstanceDetails(engineNamespace, instanceId, c) {
  try {
    ...
    for (let [, groupLogs] of stepGroups) {
      let name = (groupLogs[0].target ?? "").replace(/-\d+$/, ""), stepStart = groupLogs.find((l) => l.event === EVT.STEP_START), stepSuccess = groupLogs.find((l) => l.event === EVT.STEP_SUCCESS), stepFailure = groupLogs.find((l) => l.event === EVT.STEP_FAILURE), sleepStart = groupLogs.find((l) => l.event === EVT.SLEEP_START), sleepComplete = groupLogs.find(
