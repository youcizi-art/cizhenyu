...
var ImageHandleImpl = class extends RpcTarget {
  #imageId;
  #store;
  constructor(imageId, store) {
    super(), this.#imageId = imageId, this.#store = store;
  }
  async details() {
    return (await this.#store.getWithMetadata(
      this.#imageId,
      "arrayBuffer"
    )).metadata ?? null;
  }
  async bytes() {
    let data = await this.#store.get(this.#imageId, "arrayBuffer");
    return data === null ? null : new Blob([data]).stream();
  }
  async update(options) {
    let existing = await this.#store.getWithMetadata(
      this.#imageId,
      "arrayBuffer"
    );
    ...
    return await this.#store.put(this.#imageId, existing.value, {
      metadata: updatedMetadata
    }), updatedMetadata;
  }
  async delete() {
    return await this.#store.get(this.#imageId, "arrayBuffer") === null ? !1 : (await this.#store.delete(this.#imageId), !0);
  }
